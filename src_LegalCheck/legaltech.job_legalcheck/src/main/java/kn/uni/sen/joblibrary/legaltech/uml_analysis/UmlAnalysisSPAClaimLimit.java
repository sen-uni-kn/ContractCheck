...
public class UmlAnalysisSPAClaimLimit extends UmlAnalysisSmtAbstract
{
	...
	public void runAnalysis(ReportResult report, String statisticsFile)
	{
		...
		if (res != null)
		{
			...
			{
				result = 1;
				reportUnsat(name, "Claim limitation after due", res.getUnsatCore(), UmlResultState.GOOD);
			} else
			{
				result = 0;
				reportRun(name, "Claim limitation before due", res.getDiagram(), UmlResultState.ERROR);
			}
		}
		log(statisticsFile);
	}
